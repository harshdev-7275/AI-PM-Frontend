...
export default function AIAssistantPage() {
  ...
  return (
    <div className="flex flex-col h-full min-w-0 overflow-hidden">
      ...
      {/* ------------------------------------------------------------------ */}
      {/* Composer — suggestions row, then project + input + send             */}
      {/* ------------------------------------------------------------------ */}
      <div className="border-t border-border bg-background px-6 pt-3 pb-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {SUGGESTED.map((q) => (
                <button
                  ...
                  className="text-left text-xs px-2.5 py-1.5 rounded-full border border-border bg-background hover:bg-muted/60 hover:border-brand-primary/40 transition-colors text-muted-foreground hover:text-foreground"
                >
                  {q}
                </button>
              ))}
        ...
        {/* Input row: project dropdown · textarea · send */}
        <div className="flex gap-2 items-end">
          {/* Project selector — compact, beside the input */}
          <Select value={selectedProjectId} onValueChange={handleProjectChange}>
            <SelectTrigger
              className="h-[44px] w-[180px] shrink-0 text-xs"
